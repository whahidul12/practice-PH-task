import { colonRegex, createApplyRegex, createRegex, dynamicSyntaxMarkers, } from "../lib/regex.js";
/**
 ...
 */
export default function sortTailwind(text, sortConfig, pseudoClasses, customPrefixes) {
    ...
    text = text.replace(regex, (match, g1, g2, doubleQuotesGroup, singleQuotesGroup, backtickQuotesGroup) => {
        const quotesGroup = singleQuotesGroup || doubleQuotesGroup || backtickQuotesGroup;
 ...
 * @param pseudoClasses - An array of pseudo classes to sort by.
 */
function sortFoundTailwind(match, classesStr, sortConfig, pseudoClasses) {
    if (!classesStr || !classesStr.includes(" ")) {
        return match;
    }
    const groupContainsDynamicSyntax = dynamicSyntaxMarkers.some((syntax) => classesStr.includes(syntax));
    ...
    const { unsortedClasses, ignoredClasses } = classes.reduce((acc, className) => {
        const match = findLongestMatch(className, sortConfig);
        match
            ? acc.unsortedClasses.push(className)
            : acc.ignoredClasses.push(className);
        return acc;
    }, { unsortedClasses: [], ignoredClasses: [] });
    const sortedClasses = unsortedClasses.sort((aClass, bClass) => {
 ...
 */
export function findLongestMatch(styleClass, sortConfig) {
    const baseClass = styleClass.split(colonRegex).pop() || styleClass;
    let longestMatch = "";
    for (const key in sortConfig) {
        const keyInStyleClass = baseClass.startsWith(key) ||
 ...
 */
function comparePseudoClasses(aClass, bClass, pseudoClasses) {
